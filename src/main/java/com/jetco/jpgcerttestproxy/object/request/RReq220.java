...
public class RReq220 extends RReq{
	
	static final Logger log = LoggerFactory.getLogger(RReq210.class);
	
	private String whiteListStatus; // Whitelist Status (Optional Field)
	private String whiteListStatusSource; // Whitelist Status Sources (Optional Field)

	public Map<String, String> validateMsg(PArq pArq, ARes aRes) {
		
		log.info("In rReq220.validateMsg!!");

		Map<String, String> erro = new HashMap<String, String>();
		
		ARes220 aRes220 = (ARes220) aRes;
		
		// Message Version
		if (StringUtils.isEmpty(messageVersion)) {
			log.error("RReq v2.2.0: Message Version Number missing");
		...
		if (!ThreeD20Utils.isValidMessageVersion220(messageVersion)) {
			log.error("RReq v2.2.0: Invalid Message Version Number");
		...
		if (StringUtils.isEmpty(threeDSServerTransID)) {
			log.error("RReq v2.2.0: 3DS Server Transaction ID is missing");
			erro.put(ErrorResponse.ErrorCode201.getErrorCode(), "threeDSServerTransID");
			return erro;
		}

		if (!StringUtil.isValidUUID(threeDSServerTransID)) {
			log.error("RReq v2.2.0: Invalid 3DS Server Transaction ID [" + threeDSServerTransID + "]");
		...
		if (StringUtils.isEmpty(acsTransID)) {
			log.error("RReq v2.2.0: ACS Transaction ID is missing");
			erro.put(ErrorResponse.ErrorCode201.getErrorCode(), "acsTransID");
			return erro;
		}

		if (!StringUtil.isValidUUID(acsTransID)) {
			log.error("RReq v2.2.0:  Invalid ACS Transaction ID [" + acsTransID + "]");
		...
		if (acsRenderingType != null) {
			...
			if (StringUtils.isEmpty(acsInterface)) {
				log.error("RReq v2.2.0: ACS Interface in ACS Rendering Type is missing");
				erro.put(ErrorResponse.ErrorCode201.getErrorCode(), "acsInterface");
				return erro;
			}

			if (acsInterface.length() != 2) {
				log.error("RReq v2.2.0: Invalid length in ACS Interface in ACS Rendering Type");
			...
			if (!acsInterface.equals(ThreeD20Utils.ACS_INTERFACE_NATIVE_UI)
				...
				log.error("RReq v2.2.0:  Invalid ACS Interface in ACS Rendering Type [" + acsInterface + "]");
				erro.put(ErrorResponse.ErrorCode203.getErrorCode(), "acsInterface");
				return erro;
			}
			...
			if (StringUtils.isEmpty(acsUiTemplate)) {
				log.error("RReq v2.2.0: ACS UI Template in ACS Rendering Type is missing");
				erro.put(ErrorResponse.ErrorCode201.getErrorCode(), "acsUiTemplate");
				return erro;
			}

			if (acsUiTemplate.length() != 2) {
				log.error("RReq v2.2.0: Invalid length in ACS UI Template in ACS Rendering Type");
			...
			if (!acsUiTemplate.equals(ThreeD20Utils.ACS_UI_TEMPLATE_TEXT)
				...
				log.error("RReq v2.2.0:  Invalid ACS UI Template in ACS Rendering Type [" + acsUiTemplate + "]");
				erro.put(ErrorResponse.ErrorCode203.getErrorCode(), "acsUiTemplate");
				return erro;
			}
		} else {
			 ...
			 */
			if(pArq != null) {
				if (pArq.getDeviceChannel().equals(DeviceChannel.APP.getValue()) && !aRes220.getAcsDecConInd().equals("Y")) {
					log.error("RReq v2.2.0: ACS Interface in ACS Rendering Type is missing");
		...
		if (transStatus.equals(ThreeD20Utils.TRANS_STATUS_SUCCESS)
			...
			if (StringUtils.isEmpty(authenticationType)) {
				log.error("RReq v2.2.0: Authentication Type is missing");
				erro.put(ErrorResponse.ErrorCode201.getErrorCode(), "transStatus");
				return erro;
			}
		}
		...
		if (authenticationType != null) {
			if (authenticationType.length() != 2) {
				log.error("RReq v2.2.0: Invalid length in Authentication Type");
				erro.put(ErrorResponse.ErrorCode203.getErrorCode(), "authenticationType");
				return erro;
			}

			if (!StringUtils.isNumeric(authenticationType)) {
				log.error("RReq v2.2.0: Invalid Authentication Type [" + authenticationType + "]");
				erro.put(ErrorResponse.ErrorCode203.getErrorCode(), "authenticationType");
				return erro;
			}
		...
		if (!StringUtils.isEmpty(interactionCounter)) {
			if (interactionCounter.length() != 2) {
				log.error("RReq v2.2.0: Invalid length in Interaction Counter");
				erro.put(ErrorResponse.ErrorCode203.getErrorCode(), "interactionCounter");
				return erro;
			}

			if (!StringUtils.isNumeric(interactionCounter)) {
				log.error("RReq v2.2.0: Interaction Counter is not numeric");
				erro.put(ErrorResponse.ErrorCode203.getErrorCode(), "interactionCounter");
				return erro;
			}
		} else {
			//Interaction Counter should be required for RReq - Start
			//log.error("RReq v2.2.0: Interaction Counter is not numeric");
			//erro.put(ErrorResponse.ErrorCode201.getErrorCode(), "interactionCounter");
			//return erro;
			//Interaction Counter should be required for RReq - End
		}

		// Electronic Commerce Indicator (ECI)
		if (eci != null) {
			if (eci.length() != 2) {
				log.error("RReq v2.2.0: Invalid length in Electronic Commerce Indicator (ECI)");
				erro.put(ErrorResponse.ErrorCode203.getErrorCode(), "eci");
				return erro;
			}

			if (!StringUtils.isNumeric(eci)) {
				log.error("RReq v2.2.0: Electronic Commerce Indicator (ECI) [" + eci + "] is not numeric");
				erro.put(ErrorResponse.ErrorCode203.getErrorCode(), "eci");
				return erro;
			}

			if (!ThreeD20Utils.isValidEci(CardType.TYPE_VISA.getValue(), eci)) {
				log.error("RReq v2.2.0: Invalid Electronic Commerce Indicator (ECI) [" + eci + "]");
				erro.put(ErrorResponse.ErrorCode203.getErrorCode(), "eci");
				return erro;
			}
		}
		...
		if (StringUtils.isEmpty(dsTransID)) {
			log.error("RReq v2.2.0: DS Transaction ID is missing");
			erro.put(ErrorResponse.ErrorCode201.getErrorCode(), "dsTransID");
			return erro;
		}

		if (!StringUtil.isValidUUID(dsTransID)) {
			log.error("RReq v2.2.0:  Invalid DS Transaction ID [" + dsTransID + "]");
		...
		if (!StringUtils.isEmpty(challengeCancel)) {
			if (challengeCancel.length() != 2) {
				log.error("RReq v2.2.0: Invalid length in Challenge Cancel");
				erro.put(ErrorResponse.ErrorCode203.getErrorCode(), "challengeCancel");
				return erro;
			}

			if (!StringUtils.isNumeric(challengeCancel)) {
				log.error("RReq v2.2.0:  Invalid Challenge Cancel [" + dsTransID + "]");
				erro.put(ErrorResponse.ErrorCode203.getErrorCode(), "challengeCancel");
				return erro;
			}

			if (!ThreeD20Utils.isValidChallengeCancelationInd(challengeCancel, messageVersion)) {
				log.error("RReq v2.2.0:  Invalid Challenge Cancel [" + challengeCancel + "]. MessageVersion ["
						+ messageVersion + "].");
				erro.put(ErrorResponse.ErrorCode203.getErrorCode(), "challengeCancel");
				return erro;
			}
		...
		if (!StringUtils.isEmpty(transStatusReason) && transStatusReason.equals(ThreeD20Utils.TRANS_STATUS_REASON_14)) {
			...
			if (StringUtils.isEmpty(challengeCancel)) {
				log.error("RReq v2.2.0: Challenge Cancel is missing");
				...
				log.error("RReq v2.2.0:  Invalid combination for transStatusReason [" + transStatusReason
		...
		if (!messageCategory.equals(ThreeD20Utils.MESSAGE_CATEGORY_02)) { // merge to production
			...
			if (transStatus.equals(ThreeD20Utils.TRANS_STATUS_SUCCESS)
				...
				if (StringUtils.isEmpty(authenticationValue)) {
					log.error("RReq v2.2.0: Authentication Value	is missing");
		...
		if (whiteListStatus != null) {
			...
			if (!ThreeD20Utils.isValidWhiteListStatus(whiteListStatus, messageVersion)) {
				log.error("RReq v2.2.0: Invalid Whitelist Status [" + whiteListStatus + "], messageVersion ["
						+ messageVersion + "].");
				erro.put(ErrorResponse.ErrorCode203.getErrorCode(), "whiteListStatus");
				return erro;
			}
		...
		if (whiteListStatusSource != null) {
			if (whiteListStatusSource.length() != 2) {
				log.error("RReq v2.2.0: Invalid length in Whitelist Status Sources");
			...
			if (!ThreeD20Utils.isValidWhiteListStatusSource(whiteListStatusSource, messageVersion)) {
				log.error("RReq v2.2.0: Invalid Whitelist Status Source [" + whiteListStatusSource
						+ "], messageVersion [" + messageVersion + "].");
				erro.put(ErrorResponse.ErrorCode203.getErrorCode(), "whiteListStatusSource");
				return erro;
			}
		}
		//no need merge to production - end

		if (messageExtension != null) {
			if (messageExtension.length > 10) {
				log.error("RReq v2.2.0: Invalid array size in Message Extension");
			...
			for (int i = 0; i < messageExtension.length; i++) {
				if (messageExtension[i].getName().length() > 64) {
					log.error("RReq v2.2.0: Invalid length on name in Message Extension");
					erro.put(ErrorResponse.ErrorCode203.getErrorCode(), "messageExtension");
					return erro;
				}
				if (messageExtension[i].getId().length() > 64) {
					log.error("RReq v2.2.0: Invalid length on id in Message Extension");
					erro.put(ErrorResponse.ErrorCode203.getErrorCode(), "messageExtension");
					return erro;
				}
