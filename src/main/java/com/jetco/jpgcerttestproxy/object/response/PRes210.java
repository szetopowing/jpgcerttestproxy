...
public class PRes210 extends PRes{
	...
	public Map<String, String> validateMsg() {
		...
		if (StringUtils.isEmpty(threeDSServerTransID)) {
			log.error("PRes v2.1.0: 3DS Server Transaction ID is missing");
			erro.put(ErrorResponse.ErrorCode201.getErrorCode(), "threeDSServerTransID");
			return erro;
		}

		if (!StringUtil.isValidUUID(threeDSServerTransID)) {
			log.error("PRes v2.1.0: Invalid 3DS Server Transaction ID [" + threeDSServerTransID + "]");
			erro.put(ErrorResponse.ErrorCode201.getErrorCode(), "threeDSServerTransID");
			return erro;
		}
		...
		if (StringUtils.isEmpty(dsEndProtocolVersion)) {
			log.error("PRes v2.1.0: DS End Protocol Version is missing");
			erro.put(ErrorResponse.ErrorCode201.getErrorCode(), "dsEndProtocolVersion");
			return erro;
		}

		if (dsEndProtocolVersion.length() > 8) {
			log.error("PRes v2.1.0: Invalid length in DS End Protocol Version [" + dsEndProtocolVersion + "]");
			erro.put(ErrorResponse.ErrorCode201.getErrorCode(), "dsEndProtocolVersion");
			return erro;
		}
		...
		if (StringUtils.isEmpty(dsStartProtocolVersion)) {
			log.error("PRes v2.1.0: DS Start Protocol Version is missing");
			erro.put(ErrorResponse.ErrorCode201.getErrorCode(), "dsStartProtocolVersion");
			return erro;
		}

		if (dsStartProtocolVersion.length() > 8) {
			log.error("PRes v2.1.0: Invalid length in DS Start Protocol Version [" + dsStartProtocolVersion + "]");
			erro.put(ErrorResponse.ErrorCode201.getErrorCode(), "dsStartProtocolVersion");
			return erro;
		}
		...
		if (StringUtils.isEmpty(dsTransID)) {
			log.error("PRes v2.1.0: DS Transaction ID is missing");
			erro.put(ErrorResponse.ErrorCode201.getErrorCode(), "dsTransID");
			return erro;
		}

		if (!StringUtil.isValidUUID(dsTransID)) {
			log.error("PRes v2.1.0:  Invalid DS Transaction ID [" + dsTransID + "]");
		...
		if (serialNum != null) {
			if (serialNum.length() <= 0 || serialNum.length() > 20) {
				log.error("PRes v2.1.0: Invalid length in Serial Number [" + serialNum + "].");
		...
		if (cardRangeData != null && cardRangeData.size() == 0) {
			log.error("PRes v2.1.0: Invalid cardRangeDataList.");
		...
		if (cardRangeData != null && cardRangeData.size() > 0) {
			for (CardRangeData cardRange : cardRangeData) {
				...
				if (threeDSMethodURL != null && (threeDSMethodURL.length() <= 0 || threeDSMethodURL.length() > 256)) {
					log.error("PRes v2.1.0: Invalid length in 3DS Method URL [" + threeDSMethodURL + "].");
				...
				if (threeDSMethodURL != null) {
					...
					try {
						...
						log.error("PRes v2.1.0: Invalid 3DS Method URL [" + threeDSMethodURL + "].");
						erro.put(ErrorResponse.ErrorCode203.getErrorCode(), "threeDSMethodURL");
						return erro;
					}
				...
				if (StringUtils.isEmpty(acsEndProtocolVersion)) {
					log.error("PRes v2.1.0: ACS End Protocol Version (in Card Range Data) is missing");
				...
				if (acsEndProtocolVersion.length() < 5 && acsEndProtocolVersion.length() > 8) {
					log.error("PRes v2.1.0: Invalid length in ACS End Protocol Version [" + acsEndProtocolVersion + "].");
					erro.put(ErrorResponse.ErrorCode203.getErrorCode(), "acsEndProtocolVersion");
					return erro;
				}

				if (!acsEndProtocolVersion.matches("^\\d\\.\\d(\\.\\d)*$")) {
					log.error("PRes v2.1.0: Invalid ACS End Protocol Version [" + acsEndProtocolVersion + "].");
					erro.put(ErrorResponse.ErrorCode203.getErrorCode(), "acsEndProtocolVersion");
					return erro;
				}
				...
				if (StringUtils.isEmpty(acsStartProtocolVersion)) {
					log.error("PRes v2.1.0: ACS Start Protocol Version (in Card Range Data) is missing");
				...
				if (acsStartProtocolVersion.length() < 5 && acsStartProtocolVersion.length() > 8) {
					log.error("PRes v2.1.0: Invalid length in ACS Start Protocol Version [" + acsStartProtocolVersion
							+ "].");
				...
				if (!acsStartProtocolVersion.matches("^\\d\\.\\d(\\.\\d)*$")) {
					log.error("PRes v2.1.0: Invalid ACS Start Protocol Version [" + acsStartProtocolVersion + "].");
					erro.put(ErrorResponse.ErrorCode203.getErrorCode(), "acsStartProtocolVersion");
					return erro;
				}
				...
				if (!StringUtils.isEmpty(actionInd) && !ThreeD20Utils.isValidActionIndicator(actionInd)) {
					log.error("PRes v2.1.0: Invalid Action Indicator (in Card Range Data) [" + actionInd + "].");
				...
				if (!StringUtils.isEmpty(dsEndProtocolVersion)) {
					...
					if (!dsEndProtocolVersion.matches("^\\d\\.\\d(\\.\\d)*$")) {
						log.error("PRes v2.1.0: Invalid DS End Protocol Version [" + dsEndProtocolVersion + "].");
						erro.put(ErrorResponse.ErrorCode203.getErrorCode(), "dsEndProtocolVersion");
						return erro;
					}
				...
				if (dsStartProtocolVersion != null) {
					...
					if (!dsStartProtocolVersion.matches("^\\d\\.\\d(\\.\\d)*$")) {
						log.error("PRes v2.1.0: Invalid DS Start Protocol Version [" + dsStartProtocolVersion + "].");
						erro.put(ErrorResponse.ErrorCode203.getErrorCode(), "dsStartProtocolVersion");
						return erro;
					}
				...
				if (startBig.compareTo(endBig) == 1) {
					log.error("PRes v2.1.0: Start Card Range [" + startRange + "] is greater then End Card Range ["
							+ endRange + "].");
...
//						List<String> acsInformationIndList = cardRange.getAcsInfoInd();
//						if (acsInformationIndList != null && acsInformationIndList.size() > 0) {
//							for (String acsInformationInd: acsInformationIndList) {
//								
//								if (StringUtils.isEmpty(acsInformationInd)) {
//									log.error("PRes v2.1.0: Invalid ACS Informcation Indicator (in Card Range Data) [" + acsInformationInd + "]");
//									throw new Invalid3DMsgException("CardRangeData.acsInformationInd");
//								}
//								
//								if (acsInformationInd.length() != 2) {
//									log.error("PRes v2.1.0: Invalid length in ACS Informcation Indicator (in Card Range Data) [" + acsInformationInd + "]");
//									throw new Invalid3DMsgException("CardRangeData.acsInformationInd");
//								}
//								
//								if (ThreeD20Utils.isValidACSInfoIndicator(acsInformationInd)) {
//									log.error("PRes v2.1.0: Invalid ACS Informcation Indicator (in Card Range Data) [" + acsInformationInd + "].");
//									throw new Invalid3DMsgException("CardRangeData.acsInformationInd");
//								}
//							}
//						}
		...
		if (messageExtension != null) {
			if (messageExtension.length > 10) {
				log.error("PRes v2.1.0: Invalid array size in Message Extension");
			...
			for (int i = 0; i < messageExtension.length; i++) {
				if (messageExtension[i].getName().length() > 64) {
					log.error("PRes v2.1.0: Invalid length on name in Message Extension");
					erro.put(ErrorResponse.ErrorCode203.getErrorCode(), "messageExtension.name");
					return erro;
				}
				if (messageExtension[i].getId().length() > 64) {
					log.error("PRes v2.1.0: Invalid length on id in Message Extension");
